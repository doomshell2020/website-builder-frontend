...
export const HeroSlider = ({ company, slug }: DefaultProps): JSX.Element => {
    ...
    useEffect(() => {
        ...
        // ✅ Proper cleanup that returns void
        return () => {
    ...
    return (
        <div className="relative w-full animate-fade-in translate-y-[-1rem] mt-24">
            {/* 🖼️ Main Embla Carousel */}
            <div className="overflow-hidden" ref={emblaRef}>
                <div className="flex min-h-[80hv]">
                    {sliderImages.map((image, index: number) => (
                        ...
                        >
                            <img
                                src={image}
                                alt={`Slide ${index + 1}`}
                                className="w-full min-h-[620px] h-[514px] object-cover"
                            />
                        </div>
                    ))}
                </div>
