...
import { findFAQsCategories, deleteFAQCategory, deleteMultipleFAQCategory, updateFAQCategoryStatus } from "@/services/faqcategory.service";
...
export default function FAQCategoryListPage() {
    ...
    const [filteredData, setFilteredData] = useState<FAQCategoryAttributes[]>([]);
    ...
    const fetchData = useCallback(async () => {
        try {
            setLoading(true);
            const res: any = (await findFAQsCategories(page, limit)) as FAQCategoryAttributes[];
            const data: FAQCategoryAttributes[] = (res?.result?.data || []);
    ...
    const handleStatusChange = async (id: number, currentStatus: string) => {
        ...
        if (result.isConfirmed) {
            try {
                await updateFAQCategoryStatus(id, { status: newStatus });
                Swal.fire("Updated!", "FAQ Category status has been changed.", "success",);
                fetchData();
            } catch {
    ...
    const handleDelete = async (id: number) => {
        const result = await Swal.fire({
            title: "Are you sure",
            text: "You want to delete this FAQ category?",
            icon: "warning",
            showCancelButton: true,
        ...
        if (result.isConfirmed) {
            try {
                ...
                Swal.fire("Deleted!", "FAQ category has been deleted.", "success");
                fetchData();
            } catch {
                Swal.fire("Error!", "Failed to delete FAQ category.", "error");
            }
        }
    };
    ...
    const handleBulkDelete = async () => {
        ...
        const result = await Swal.fire({
            title: "Are you sure?",
            text: `You want to delete ${selectedIds.length} FAQ category?`,
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#506ae5",
            cancelButtonColor: "#d33",
        ...
        if (result.isConfirmed) {
            try {
                await deleteMultipleFAQCategory(selectedIds.map(String)); // Send all at once
                Swal.fire("Deleted!", "Selected FAQ categories have been deleted.", "success");
                setSelectedIds([]);
                fetchData();
            } catch {
    ...
    const handleClick = () => { setIsLoading(true); router.push("/user/faqs/categories/add") };
        ...
        () => [
            {
                ...
                cell: (row: FAQCategoryAttributes) => (
                    <input
                        type="checkbox"
            ...
            {
                name: "S.No",
                selector: (_: FAQCategoryAttributes, index: number) => (page - 1) * limit + index + 1,
                width: "5%",
            },
            {
                name: "Category Name",
                selector: (row: FAQCategoryAttributes) => row.name || "N/A",
                width: "20%",
            },
            {
                name: "Description",
                selector: (row: FAQCategoryAttributes) => row.description || "N/A",
            },
            {
                name: "Created",
                selector: (row: FAQCategoryAttributes) => row.createdAt ? formatDate(row.createdAt, "DD-MM-YYYY") : "—",
                width: "10%",
            },
            {
                name: "Actions",
                cell: (row: FAQCategoryAttributes) => (
                    <div className="flex space-x-3 items-center">
                        ...
                        <button
                            title="Edit"
                            onClick={() => router.push(`/user/faqs/categories/edit/${row.id}`)}
                        >
