...
export default function FaqAddForm() {
    ...
    const fetchCategories = async () => {
        try {
            ...
            // const activeData = categories.filter(item => item.status === 'Y');
            // setFAQCategories(activeData);
    ...
    const onSubmit = async (data: FormData) => {
        try {
            ...
            const response: any = await createFAQ(payload);
            if (response?.status === true) { SwalSuccess("FAQ added successfully."); handleBack(); }
            ...
            <header className="bg-white shadow-sm border-b">
                <div className="mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center h-12">
                        <h1 className="text-xl font-medium text-gray-800 ml-2">Add FAQs</h1>
                    </div>
                </div>
            </header>
